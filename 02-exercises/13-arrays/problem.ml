...
let double array : unit = Array.iteri array ~f:(fun ind x -> Array.set array ind (x * 2))
...
let double_selectively array indices : unit = 
  List.iter indices ~f:(fun ind -> Array.set array ind ((Array.get array ind) * 2))
...
let double_matrix matrix : unit =
  Array.iteri matrix ~f:(fun col row -> Array.iteri row ~f:(fun ind x -> matrix.(col).(ind) <- (x * 2) ))
